...
- Start 后读取手机 GPS。
- 默认每 1 秒请求定位；确认有实际移动后保存 `lng / lat / timestamp / accuracy / speed / heading / altitude`。
- 原地不动时会忽略 stationary GPS drift；低速设备 `speed` 需要约 6-12 米的连续位移确认，达到正常走路速度时会更快开始记录，过旧或早于 Start 的 GPS fix 也不会保存成路线点。
